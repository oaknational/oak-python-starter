...
Reusable patterns proven in real repo work. These are repo-local instances:
more concrete than rules, and more local than the portable Core patterns in
`.agent/practice-core/patterns/`.

## Relationship to the Practice Core

- `.agent/memory/active/patterns/` holds repo-grounded reusable patterns proved
  in this template.
- `.agent/practice-core/patterns/` is reserved for portable general patterns.
- Practice-governance doctrine belongs in
...
Imported Practice-derived instances are allowed when they arrive through a
deliberate Practice integration. Keep their provenance truthful and revisit
them once local evidence accumulates.
